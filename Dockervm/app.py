...
# Define PORT and STORAGE_DIR early so they are available in all routes.
...
# Generate (or reuse) a persistent NODE_ID.
...
def register_node_in_supabase():
    # Determine IP based on role: Relay prefers IPv6 if available; Adept uses IPv4.
    if ROLE.lower() == "relay":
    ...
    data = {
        "id": NODE_ID,
        "nickname": NODE_NICKNAME,  # This value will not be updated if the node already exists.
        "role": ROLE,
        "ip_address": real_ip,
        "active": True,
    ...
    # Use upsert headers so that if a duplicate exists, it can merge or update.
    headers = {
    ...
    try:
        ...
        if r.status_code in (200, 201):
            ...
            # Issue a PATCH request to update dynamic fields only.
            patch_headers = {
                "apikey": SUPABASE_API_KEY,
                "Content-Type": "application/json",
            ...
            patch_data = {
                "ip_address": real_ip,
                "active": True
                # Note: We intentionally leave out "nickname" so it remains unchanged.
            }
...
# Register our own node in the DHT.
if ROLE.lower() == "adept":
    ips = get_ipv4_addresses()
else:
    ips = get_ipv6_addresses() or get_ipv4_addresses()
